...
interface Task { id: string; title: string; by_id?: string; assignee_id: string; status: string; start: string | null; due: string | null; body: string; link?: string; files?: TFile[]; }
...
// 모듈 레벨로 정의 — 컴포넌트 내부에 두면 매 렌더마다 새 타입으로 인식되어 input 포커스가 풀린다.
...
export default function Projects({ kind = "grant" }: { kind?: "grant" | "activity" }) {
  ...
  const { me } = useAuth();
  // 생성 권한: 연구과제=교수·위임 / 활동=구성원 누구나. 수정·삭제: 교수·위임(연구과제) + 책임자·담당자(활동). 세부업무: 교수·행정·위임 또는 참여자.
  ...
  const emptyForm = {
    ...
    budget_total: "", budget_year: "", ack_ko: "", ack_en: "", link: "", extra: "", desc: "",
  ...
  // 칸반 카드 영역 높이를 창 하단까지 맞추되, 페이지(.content) 스크롤이 생기지 않게 보정
  // → 안쪽 컬럼 스크롤 하나만 남기고 전체 스크롤은 띄우지 않음
  useEffect(() => {
    let r1 = 0, r2 = 0;
    function fit() {
  ...
  function fillForm(p: Project) {
    ...
    setForm({
      ...
      ack_ko: mm.ack_ko || "", ack_en: mm.ack_en || "", link: mm.link || "", extra: mm.extra || "", desc: p.desc || "",
  ...
  async function save(e: React.FormEvent) {
    ...
    const payload = isGrant ? {
      kind: "grant", code: form.code, name: form.name, category: "과제", status: autoStatus(form.start, form.end),
      ...
      meta: {
        ...
        ack_ko: form.ack_ko, ack_en: form.ack_en,
      },
    } : {
  ...
  function newTask() { setTaskOpen(null); setTaskForm({ title: "", assignee_id: "", status: "예정", start: "", due: "", link: "", files: [] }); setTaskBody(""); }
  function editTask(t: Task) { setTaskOpen(null); setTaskForm({ id: t.id, title: t.title, assignee_id: t.assignee_id || "", status: t.status, start: t.start || "", due: t.due || "", link: t.link || "", files: t.files || [] }); setTaskBody(t.body || ""); }
  ...
  async function saveTask(e: React.FormEvent) {
    ...
    const payload = { title: taskForm.title, assignee_id: taskForm.assignee_id, status: taskForm.status, start: taskForm.start || null, due: taskForm.due || null, body: taskBodyRef.current?.innerHTML || "", link: taskForm.link, files: taskForm.files || [] };
    try {
  ...
  async function moveTask(t: Task, status: string) {
    ...
    // 낙관적 업데이트 후 서버 반영(나머지 필드는 보존)
    setTasks((ts) => ts.map((x) => (x.id === t.id ? { ...x, status } : x)));
    try {
      await api.patch(`/projects/tasks/${t.id}`, { title: t.title, assignee_id: t.assignee_id, status, start: t.start || null, due: t.due || null, body: t.body || "", link: t.link || "", files: t.files || [] });
      reloadTasks();
  ...
  function goalBars(p: Project) {
    ...
    pubs.filter((u) => u.project_id === p.id).forEach((u) => { const mk = pubMetric(u); counts[mk] = (counts[mk] || 0) + 1; });
    return Object.keys(g).map((k) => {
      const tg = g[k], ac = counts[k] || 0; const pct = tg ? Math.round(Math.min(ac, tg) / tg * 100) : 0;
      return { label: k, pct, text: `달성 ${ac} / 목표 ${tg}${ac >= tg ? " ✓" : ""}`, color: ac >= tg ? "#2e9e6b" : pct >= 50 ? "#3f5d7d" : "#c2891b" };
    });
  }
  ...
  // 수정·삭제 권한: 연구과제=교수·위임 / 활동=교수·위임 또는 책임자·담당자 (백엔드 _can_edit_pj 동일)
  const canEdit = canManageProject || (!isGrant && !!open && !!me && [open.lead_id, open.pm_id].includes(me.id));
  const liveStatus = (p: Project) => autoStatus(p.start || "", p.end || "");
              ...
              return (
                <div className={"kbcol" + (overCol === st ? " kbcol-over" : "")} key={st} style={{ ["--kc" as any]: STC[st] }}
                  ...
                  <div style={{ maxHeight: kbMax, overflowY: "auto", marginRight: -4, paddingRight: 4 }}>
                    {list.map((t) => {
                      const late = t.status !== "완료" && t.due && t.due < todayKST();
                      ...
                      return (
                        <div className={"kbcard" + (late ? " late" : "") + (dragId === t.id ? " kbcard-drag" : "")} key={t.id}
                          ...
                          <div className="kbc-m"><span>🗓 {t.due || "—"}</span>{t.assignee_id && <span>· {uname(t.assignee_id)}</span>}</div>
                        </div>
                      );
                    })}
            ...
            <div className="modal" data-testid="proj-info-modal" style={{ maxWidth: 580 }}>
              ...
              <div className="modal-b">
                {!isGrant ? <>
                  ...
                  <KV k="기간" v={`${open.start || "—"} ~ ${open.end || "—"}`} />
                  <KV k="담당자" v={uname(open.pm_id)} />
                  <KV k="관련 링크" v={m.link ? <a href={m.link} target="_blank" rel="noreferrer">{m.link}</a> : "—"} />
                  <KV k="상태" v={open.status} />
        ...
        <form className="card" onSubmit={save} data-testid="project-form">
          ...
          {isGrant ? (
            <div className="bd grid2">
              ...
              <Field label="실무 담당자"><select data-testid="p-pm" value={form.pm_id} onChange={(e) => setForm({ ...form, pm_id: e.target.value })}><option value="">선택</option>{users.filter((u) => u.role !== "admin").map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></Field>
              <Field label={`참여 연구원${form.members.length ? ` · ${form.members.length}명` : ""}`} full>
                <div className="fchips" data-testid="p-members">
                  {users.filter((u) => u.role !== "admin").map((u) => <button type="button" key={u.id} className={"chip" + (form.members.includes(u.id) ? " on" : "")} onClick={() => toggleMember(u.id)}>{u.name}</button>)}
                </div>
              </Field>
            ...
            <div className="bd grid2">
              ...
              <Field label="담당자"><select data-testid="p-pm" value={form.pm_id} onChange={(e) => setForm({ ...form, pm_id: e.target.value })}><option value="">선택</option>{users.filter((u) => u.role !== "admin").map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}</select></Field>
          ...
          <tbody>
            {shown.map((p) => (
              <tr key={p.id} style={{ cursor: "pointer" }} onClick={() => openDetail(p)}>
                ...
                <td>{p.name}</td>
                {isGrant ? <><td>{p.agency || "—"}</td><td>{p.program || "—"}</td></> : <><td>{p.category}</td><td>{uname(p.pm_id)}</td></>}
