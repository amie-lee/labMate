...
export const SHEETS: Record<string, SheetEntity> = {
  members: {
    ...
    transform: (r) => { if (r.role) r.role = ROLE_FROM_KO[r.role] || r.role; if (!r.temp_password) r.temp_password = "labmate123"; for (const k of ["birth", "join_date", "exit_date"]) if (!r[k]) delete r[k]; return r; },
  },
  grants: {
